...
class GrupoController {
    ...
    if ($ldap->crearGrupo($cn, '', $miembrosValidos, $ou)) {
        ...
        // Mostrar información sobre todos los miembros agregados
        if (!empty($miembrosValidos)) {
            $cantidadMiembros = count($miembrosValidos);
            
            if ($cantidadMiembros == 1) {
                $_SESSION['success'] .= " Se agregó el usuario '" . $miembrosValidos[0] . "' como miembro inicial.";
            } else {
                $_SESSION['success'] .= " Se agregaron $cantidadMiembros usuarios como miembros iniciales: " . implode(', ', $miembrosValidos) . ".";
            }
        }
    } else {
